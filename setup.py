...
load_dotenv() #need to clarify what's this
...
def setup(pl1=PLAYERS_LEVEL_1, pl2=PLAYERS_LEVEL_2):
    """Sets up players with data structure"""
    pm1 = PlayersMap(pl1)
    pm2 = PlayersMap(pl2)
    return PlayersMap(dict(pm1.get_players_map(), **pm2.get_players_map()))
    return (
        pm1,
        pm2,
        pm3,
...
if __name__ == "__main__":
    ...
    players = setup(PLAYERS_LEVEL_1, PLAYERS_LEVEL_2)

    [print(i) for i in players.get_players_map().items()]
    player1 = players.get('IsaacPoh')
